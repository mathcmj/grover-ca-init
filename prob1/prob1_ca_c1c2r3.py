...
def unitary_Dicke_leone(circ, q_list):
    n = len(q_list)
    circ.ry(2*np.arccos(np.sqrt(1/(n+1))), q_list[-1])
    for iter in range(n-1):
        scs_n_1(circ=circ, m=n-iter, i=q_list[n-2-iter])
def inv_unitary_Dicke_leone(circ, q_list):
    n = len(q_list)
    for iter in reversed(range(n-1)):
        inv_scs_n_1(circ=circ, m=n-iter, i=q_list[n-2-iter])
    circ.ry(-2*np.arccos(np.sqrt(1/(n+1))), q_list[-1])
...
def reflection():
    inv_unitary_W(qc_Grover, [0, 1, 2, 3])
    inv_unitary_W(qc_Grover, [4, 5, 6])
    inv_unitary_Dicke_leone(qc_Grover, [7, 8, 9])
    qc_Grover.barrier()
    for i in range(n_s):
        qc_Grover.x(i)
    qc_Grover.h(n_s-1)
    ...
    qc_Grover.z(0)
    qc_Grover.barrier()
    unitary_W(qc_Grover, [0, 1, 2, 3])
    unitary_W(qc_Grover, [4, 5, 6])
    unitary_Dicke_leone(qc_Grover, [7, 8, 9])
    qc_Grover.barrier()
...
qc_Grover.barrier()
# iter = 5
iter = round((np.pi/(4*np.arcsin(np.sqrt(1/(4*3*4)))))-1/2)
print(iter)
for i in range(iter):
    oracle()
    reflection()
...
noise_model.add_all_qubit_quantum_error(single_qubit_error, ['ry', 'rz', 'z', 'h', 'x'])
noise_model.add_all_qubit_quantum_error(two_qubit_error, ['cx', 'cry'])

f_ideal = open('results_prob1_ca_c1c2r3_ideal', 'a')
f_noisy = open('results_prob1_ca_c1c2r3_noisy', 'a')
