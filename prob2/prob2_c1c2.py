...
def unitary_ghz(circ, q_list):
    n = len(q_list)
    circ.h(q_list[-1])
    for i in range(n-1):
        circ.cx(q_list[n-1-i], q_list[n-2-i])
    for i in range(n):
        circ.h(q_list[i])
def inv_unitary_ghz(circ, q_list):
    n = len(q_list)
    for i in range(n):
        circ.h(q_list[i])
...
def reflection():
    inv_unitary_ghz(qc_Grover, [1, 4])
    inv_unitary_ghz(qc_Grover, [6, 7])
    for i in [0, 2, 3, 5, 8, 9]:
        qc_Grover.h(i)
    qc_Grover.barrier()
    for i in range(n_s):
        qc_Grover.x(i)
    ...
    qc_Grover.z(0)
    qc_Grover.barrier()
    unitary_ghz(qc_Grover, [6, 7])
    unitary_ghz(qc_Grover, [1, 4])
    for i in [0, 2, 3, 5, 8, 9]:
        qc_Grover.h(i)
    qc_Grover.barrier()
...
qc_Grover.barrier()
# iter = 12
iter = round((np.pi/(4*np.arcsin(np.sqrt(1/(2**(n_s-2))))))-1/2)
print('iter:', iter)
for i in range(iter):
    oracle()
    reflection()
...
f_ideal = open('results_prob2_ca_c1c2_ideal', 'a')
f_noisy = open('results_prob2_ca_c1c2_noisy', 'a')
